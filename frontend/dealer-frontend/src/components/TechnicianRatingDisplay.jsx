...
import { FaStar, FaStarHalfAlt, FaUser, FaCalendarAlt, FaComment } from 'react-icons/fa';
import api from '../api';

const TechnicianRatingDisplay = ({ technicianEmail, showDetailed = false, className = '' }) => {
  ...
  const fetchRatingData = async () => {
    try {
      ...
      const summaryResponse = await api.get(`/api/ratings/technician/${technicianEmail}/summary`);
      setRatingSummary(summaryResponse.data);

      // If detailed view is requested, also fetch individual ratings
      if (showDetailed) {
        const ratingsResponse = await api.get(`/api/ratings/technician/${technicianEmail}`);
        setRatings(Array.isArray(ratingsResponse.data) ? ratingsResponse.data : ratingsResponse.data.content || []);
      }
    } catch (error) {
  ...
  return (
    <div className={className}>
        ...
        <Card className="rating-summary-card">
          <Card.Header className="bg-light">
            <div className="d-flex justify-content-between align-items-center">
              ...
              <Badge bg={getRatingColor(avgRating)} className="rating-quality-badge">
            ...
            <Row className="mb-4">
              <Col md={6}>
                <div className="text-center">
                  ...
                  <div className="text-muted">
                    Based on {ratingSummary.totalRatings} review{ratingSummary.totalRatings !== 1 ? 's' : ''}
                  </div>
                </div>
              </Col>
              ...
              <Col md={6}>
                ...
                <div className="rating-distribution">
                  {[5, 4, 3, 2, 1].map(star => {
                    const count = ratingSummary[`${['', 'one', 'two', 'three', 'four', 'five'][star]}StarCount`];
                    ...
                    return (
                      <div key={star} className="d-flex align-items-center mb-2">
                        <span className="me-2" style={{ minWidth: '60px' }}>
                          {star} <FaStar size={12} style={{ color: '#ffc107' }} />
                        </span>
                        <ProgressBar 
                          now={percentage} 
              ...
              <Col md={4}>
                <div className="text-center p-3 bg-light rounded">
                  <div className="fw-bold text-info">
                    {ratingSummary.lastRatedAt ? formatDate(ratingSummary.lastRatedAt) : 'N/A'}
                  </div>
              ...
              <>
                ...
                {ratings.slice(0, 3).map((rating, index) => (
                  <div key={rating.id} className="border-bottom pb-3 mb-3">
                    ...
                    <div className="mt-2">
                      <small className="text-muted">
                        Job: {rating.postTitle || 'Vehicle Inspection'} • {rating.postLocation}
                      </small>
                    </div>
                  </div>
                ))}
      ...
      {/* All Ratings Modal */}
      <Modal show={showAllRatings} onHide={() => setShowAllRatings(false)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>All Ratings for {ratingSummary.technicianName || technicianEmail}</Modal.Title>
        </Modal.Header>
        <Modal.Body style={{ maxHeight: '500px', overflowY: 'auto' }}>
          {ratings.map((rating, index) => (
            <div key={rating.id} className="border-bottom pb-3 mb-3">
              ...
              <div>
                <small className="text-muted">
                  <strong>Job:</strong> {rating.postTitle || 'Vehicle Inspection'} • {rating.postLocation}
                </small>
              </div>
            </div>
          ))}
