...
import { FaCheck, FaTimes, FaMapMarkerAlt, FaDollarSign, FaFileAlt } from "react-icons/fa";
import "./ModalStyles.css";

const SimpleAcceptModal = ({ show, onHide, onConfirm, post, isProcessing = false }) => {
  const handleConfirm = () => {
  ...
  return (
    <Modal show={show} onHide={handleCancel} centered size="md" className="pro-modal pro-modal-accept">
      <Modal.Header>
        <div className="pro-icon-circle me-2">
          <FaCheck className="text-white" />
        </div>
        <Modal.Title>Accept Post</Modal.Title>
      ...
      <Modal.Body>
        <div className="text-center mb-3">
          <div className="text-body-secondary mb-2">Are you sure you want to accept this post?</div>
        </div>
      ...
      <Modal.Footer className="pro-footer">
        ...
        <Button className="pro-btn-primary" onClick={handleConfirm} disabled={isProcessing}>
          {isProcessing ? (
            <>
              <span className="spinner-border spinner-border-sm me-2" /> Accepting...
            </>
          ) : (
            <>
...
export default SimpleAcceptModal;
